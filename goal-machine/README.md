...
**Soundtrack:** Settings → Music has a third option, 🎧 Soundtrack, which plays real recorded songs from `music/` on shuffle instead of the synthesised tracks. To add a song, upload the `.mp3` on GitHub (ideally into `music/`): the *Add songs* action moves it into `music/` and adds it to `music/playlist.json`, taking the title and artist from Epidemic Sound file names (`ES_Title - Artist.mp3`). You can also run `python3 goal-machine/tools/add_songs.py` yourself, or edit `playlist.json` by hand. The song credits on the About page come from the same list. Only add music that is licensed for use in the game; keep files around 5 MB (128–192 kbps) so they stream quickly.
