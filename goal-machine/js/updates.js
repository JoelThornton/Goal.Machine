/* Goal Machine – what's new. Add an entry at the top for every release (v = the ?v= number in index.html). */
'use strict';
...
(function () {
  GM.UPDATES = [
    ...
    {
      v: 12, date: '2026-09-25', app: 9, title: 'New logo',
      items: [
    ...
    {
      v: 11, date: '2026-09-25', app: 7, title: 'New home',
      items: [
    ...
    {
      v: 2, date: '2026-09-24', title: 'Ultimate Wildcard',
      items: [
    ...
    {
      v: 1, date: '2026-09-24', title: 'Kick-off',
      items: [
  ...
  const releaseCard = (u, open) => `<article class="release ${u.v === GM.VERSION ? 'current' : ''}">
      <header><span class="ver">v${u.v}</span><b>${GM.esc(u.title)}</b>
  ...
  GM.updatesPage = function (root) {
    ...
    root.innerHTML = `<div class="topbar"><a href="#/" class="back">‹</a><h2>📰 Updates</h2><span></span></div>
      <div class="version-strip">
        <div><small>Game version</small><b>v${GM.VERSION}</b></div>
      ...
      <div class="releases">${GM.UPDATES.map((u, i) => releaseCard(u, i < 3)).join('')}</div>`;
