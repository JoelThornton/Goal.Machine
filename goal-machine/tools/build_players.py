...
TMP = pd.read_csv(f'{S}/football-datasets/datalake/transfermarkt/player_profiles/player_profiles.csv', low_memory=False,
                  usecols=['player_id', 'player_name', 'date_of_birth', 'citizenship', 'position', 'main_position', 'player_image_url'])
# Transfermarkt portraits fill in faces for players the Premier League site has no photo of (mostly pre-2016).
# Kept as the bit after .../portrait/header/ without .jpg; '' for none or TM's grey default.
tm_img = [''] * len(out)
...
for i, p in enumerate(out):
    ...
    if m is not None:
        ...
        url = TMP.player_image_url[m.Index]
        if isinstance(url, str) and url.startswith(TM_IMG) and url.endswith('.jpg') and 'default' not in url:
            tm_img[i] = url[len(TM_IMG):-4]
print('TM portraits', sum(1 for x in tm_img if x), 'of', len(out), file=sys.stderr)
...
compact = dict(
    ...
    #  Transfermarkt portrait id ('' none)]
    ...
    players=[[p['name'], p['poss'],
              ...
              ''.join(f'{k}{v}' for k, v in sorted(hon[i].items())), tm_img[i]] for i, p in enumerate(out)])
OUT = os.environ.get('OUT', f'{S}/players.js')
with open(OUT, 'w') as f:
    f.write('// Generated by tools/build_players.py - do not edit by hand\n')
    # VAR=PL_ALL for the every-player file (players_all.js, built with minimum apps 1)
    f.write(f"window.{os.environ.get('VAR', 'PL_DATA')}=" + json.dumps(compact, ensure_ascii=False, separators=(',', ':')) + ';\n')
