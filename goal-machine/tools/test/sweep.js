...
(async () => {
  ...
  for (const theme of ['light', 'dark', 'club']) {
    ...
    for (const r of ['', 'today', 'leaderboard', 'album', 'players', 'settings', 'about', 'updates', 'h2h', 'online', 'footle', 'credits', 'draft?m=ultimate', 'draft?m=classic', 'draft?m=extreme', 'draft?m=target', 'hilo', 'whoami', 'grid', 'tally', 'hopper', 'moneyball', 'moneyball?s=goals', 'moneyball?daily=1', 'window?s=apps', 'auction', 'auction?new=1', 'draft?m=chaos', 'draft?m=club', 'album?s=apps', 'draft?m=chaos&daily=1', 'online']) {
    ...
    if (theme === 'club') { await pg.goto(U + '#/settings'); await pg.waitForTimeout(600); await pg.screenshot({ path: 'lay/settings_club.png', fullPage: true }); }
  }
